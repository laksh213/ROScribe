#!/usr/bin/env bash
#
# ROS Metadata Extractor — macOS Packager
# Builds a standalone .app bundle and packs it into a drag-and-drop .dmg installer.
#
# Config (all optional, via environment):
#   LLAMACPP_MODEL_PATH=/path/to/model.gguf   # overrides .env / built-in default
#   BUNDLE_MODEL=0                             # build lean (do NOT bake the model in)
...
echo "=================================================="
echo "      Starting ROS Extractor macOS Build          "
echo "=================================================="
...
# Bundle the (large) model into the app? Default yes => fully offline, self-contained.
# BUNDLE_MODEL=0 => lean build; the app loads a model at runtime from a .env beside
# it, the in-UI "Model or GGUF Path" field, or an API provider.
BUNDLE_MODEL="${BUNDLE_MODEL:-1}"
BUNDLE_ID="${BUNDLE_ID:-lk.roscribe.extractor}"
...
# 2. App icon (.icns) — generate from the logo if it is missing.
...
# 3. Run PyInstaller to bundle the application.
# ${arr[@]+"${arr[@]}"} = bash 3.2 / `set -u`-safe expansion of possibly-empty arrays.
...
.venv/bin/pyinstaller \
    ...
    ${MODEL_ARG[@]+"${MODEL_ARG[@]}"} \
    app/desktop_extractor.py

echo "PyInstaller build complete. Standalone bundle created at: dist/ROS_Extractor.app"

# Clean up Finder/macOS custom attributes and detritus to prevent codesign errors
xattr -cr dist/ROS_Extractor.app || true

# 4. Build the DMG installer using dmgbuild
echo "Generating DMG installer..."
rm -f dist/ROS_Extractor.dmg

.venv/bin/dmgbuild -s scripts/dmg_extractor_settings.py "ROS Metadata Extractor" dist/ROS_Extractor.dmg

echo "=================================================="
echo "Build Successful! DMG installer created at:"
echo "  dist/ROS_Extractor.dmg"
echo "=================================================="
